\begin{figure}[t]
    ...
    \end{subfigure}
	\caption[An example of semantic segmentation.]{An example of semantic segmentation on a color image. This example is taken from the Mapillary dataset~\cite{mapillary}. In this example, the RGB image in \figref{fig:background-raw} has been segmented by a human in order to produce the ground truth segmentation seen in \figref{fig:background-segmented}. These image pairs can be used to train and validate the performance of a deep learning based semantic segmentation model.}
\end{figure}